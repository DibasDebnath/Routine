...
?>
	...
	<form method="post" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>">  
		Rooms: <select name = "R_code">
		<?php 
		while($roomsrow = $rooms->fetch_assoc()){
			echo "<option value='$roomsrow[R_code]'>$roomsrow[R_code]</option>";	
		}
		?>
		</select>
...
<h1>Full Routine of Room <?php echo $Rcode;?></h1>
    ...
    echo "<table>
		...
		case 5 : echo "<tr><th>Thursday</th>"; break;
				...
				for ($Timeslot = 0; $Timeslot < 6; $Timeslot++) {
					...
					$sql = "SELECT * FROM timetable where R_code = '$Rcode' and Dayslot = $Dayslot and Timeslot = $Timeslot;";
					...
					echo "<th>$timetablerow[S_id]$grpno  $timetablerow[C_id]<br/>$timetablerow[T_id]</th>";
